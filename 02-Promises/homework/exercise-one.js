...
function problemC () {
  ...
  // ???
  // Then Hell --> Anidamiento de Promesas EVITAR!!
  ...
  //     blue(stanza3)
  //     console.log('done')
  // }) NO ESTA BUENO EL CODIGO

  // Encadenamiento de promesas --> cuando los then se encuentrarn en el mismo nivel
  // promisifiedReadFile('poem-one/stanza-02.txt')
  //   .then(stanza2 => {
  //     blue(stanza2)
...
function problemD () {
  ...
  // 1- Manejamos en el then. success handlen and err handle 2- sino lo hacemos en el catch

  promisifiedReadFile('poem-one/wrong-file-name.txt')
    .then(
      stanza => blue(stanza),  //successHandler
      //err => magenta(new Error(err)) // errorHandler
    ) // Catch maneja errores solo agarra err
